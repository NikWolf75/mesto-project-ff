...
import { createCard, deleteCard } from './card.js';  // импортируем createCard и deleteCard
...
// Получаем список для карточек
const placesList = document.querySelector('.places__list');

// Вывод карточек на страницу
initialCards.forEach((card) => {
  const cardElement = createCard(card, deleteCard, handleLike, handleImageClick);  // передаем handleLike
  placesList.append(cardElement);
});