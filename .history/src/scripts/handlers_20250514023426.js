...
export function handleImageClick(evt) {
  ...
  // Извлекаем название из карточки
  ...
  const cardDescription = cardElement.querySelector('.card__description') ? cardElement.querySelector('.card__description').textContent : '';  // Проверяем на наличие описания
  
  // Если описание есть, добавляем его, если нет — оставляем только название
  const textToDisplay = cardDescription ? `${cardTitle} - ${cardDescription}` : cardTitle;
  
  // Устанавливаем текст в попапе
  popupCaption.textContent = textToDisplay;
