...
logoImg.src = logo;
logoImg.alt = 'Логотип проекта место';
...
editButton.addEventListener('click', () => {
  // Здесь открываем попап с помощью openModal
  openModal(popupEdit); // Убедись, что popupEdit определен
});
...
editButton.addEventListener('click', () => {
  ...
  openModal(popupEdit);  // Поменяли openPopup на openModal
});

addButton.addEventListener('click', () => openModal(popupAddCard)); // открывает попап для добавления карточки
imageButton.addEventListener('click', () => openModal(popupAvatarEdit)); // открывает попап для смены аватара
...
function handleFormSubmit(evt) {
  ...
  // Сохраняем новые значения из формы в профиль
