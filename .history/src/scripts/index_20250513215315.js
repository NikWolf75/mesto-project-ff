...
const placesList = document.querySelector('.places__list'); // Здесь переменная объявляется один раз
export function handleFormSubmit({ button, request, onSuccess, onError = console.error }) {
...
addCardForm.addEventListener('submit', (evt) => {
  ...
  const cardElement = createCard(newCard, deleteCard, handleLike, handleImageClick); // передаем обработчик клика на изображение
...
function handleImageClickWrapper(event) {
  handleImageClick(event, openPopup);
}

// передаёшь в createCard
initialCards.forEach((card) => {
  const cardElement = createCard(card, deleteCard, handleLike, handleImageClickWrapper);
  placesList.append(cardElement);
});

const cardElement = createCard(newCard, deleteCard, handleLike, handleImageClickWrapper);