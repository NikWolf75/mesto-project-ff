...
initialCards.forEach((card) => {
  const cardElement = createCard(card, deleteCard, handleLike, handleImageClick); // используем импортированную функцию
...
// Это обеспечит, что код сработает после того, как DOM загрузится
document.addEventListener('DOMContentLoaded', () => {
  // Добавление обработчика для кнопки лайка
  const likeButton = document.querySelector('.like-button');
  if (likeButton) {
    likeButton.addEventListener('click', handleLike);
  }

  // Добавление обработчика для клика по изображению
  const images = document.querySelectorAll('.image'); // Если изображений несколько
