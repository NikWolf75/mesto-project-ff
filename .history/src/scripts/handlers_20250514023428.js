...
export function handleImageClick(evt) {
  ...
  const cardDescription = cardElement.querySelector('.card__text').textContent;  // Получаем описание из карточки
  
  // Устанавливаем описание в попап
  popupCaption.textContent = `${cardTitle} - ${cardDescription}`;  // Вставляем текст в попап
