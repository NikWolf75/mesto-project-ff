...
export function handleImageClick(evt) {
  ...
  const cardTitle = cardElement.querySelector('.card__title').textContent;  // Название карточки
  const cardDescription = cardElement.querySelector('.card__text').textContent;  // Описание карточки
  
  // Устанавливаем текст в попап
  popupCaption.textContent = `${cardTitle} - ${cardDescription}`;  // Добавляем название и описание в подпись попапа
